...
<form id="formPanier" action="index.php?module=confirmePanier" method="post"> <!-- index.php?module=confirmePanier -->
<?php
	...
	if(isset($_COOKIE['selection']) && $_COOKIE['selection'][0]!=0){
		for($i = 1 ; $i <= $_COOKIE['selection'][0] ; ++$i){
			...
			echo '<a href="index.php?module=descriptif&numFilm='.$numFilm.'">'.getTitre($numFilm).'</a>
...
<div id="option_panier">
	...
	<div id="option_panier_droite">
		<input type="submit" class="bouton" name="commande" value="Commander" onClick="panier(error)"/>
	</div>
	<br />
<?php
